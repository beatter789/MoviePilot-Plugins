...
- 配置页首行将“启用插件”“检查 Cookie”“清理缓存”并列为三个响应式操作列，桌面端各占四列，移动端自动换行；Cookie 检查按钮使用 `info` 蓝色，清理缓存保持 `warning` 样式。
- Cookie 检查状态反馈位于“检查 Cookie”按钮同一列下方，显示“尚未检查 Cookie”“正在检查 Cookie…”“Cookie 有效”或失败提示，并随结果变化颜色；底部另有一个不调用 API 的动态按钮演示，点击后每 2 秒从检查中变为“Cookie 有效”，用于验证传统渲染器的按钮文字和颜色模型绑定。
