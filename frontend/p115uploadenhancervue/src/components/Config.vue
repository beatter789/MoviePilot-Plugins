...
const local = reactive({ enabled: false, cookie: '', upload_module_enhancement: true, upload_module_wait_time: 300, upload_module_wait_timeout: 3600, upload_module_skip_upload_wait_size: '0', upload_module_force_upload_wait_size: '0', upload_module_skip_slow_upload: false, upload_module_skip_slow_upload_size: '0', ...props.config })
...
const getQr = async () => {
  ...
  try {
    const response = await call('plugin/P115UploadEnhancerVUE/get_qrcode')
...
const checkQr = async () => {
  ...
  try {
    ...
    const response = await call(`plugin/P115UploadEnhancerVUE/check_qrcode?${query}`)
    const data = response?.data || response
...
const checkCookie = async () => { message.value = '正在检查 Cookie...'; messageType.value = 'info'; try { const r = await call('plugin/P115UploadEnhancerVUE/refresh_account_status', { method: 'post' }); message.value = r?.msg || (r?.success ? 'Cookie 有效' : 'Cookie 无效'); messageType.value = r?.success ? 'success' : 'warning' } catch (e) { message.value = e.message; messageType.value = 'error' } }
const clearCache = async () => { try { const r = await call('plugin/P115UploadEnhancerVUE/clear_cache', { method: 'post' }); message.value = r?.msg || '缓存清理完成'; messageType.value = 'success' } catch (e) { message.value = e.message; messageType.value = 'error' } }
</script>
