...
const refresh = async () => {
  ...
  loading.value = true
  try { status.value = await props.api.post('plugin/P115UploadEnhancer/refresh_account_status') } catch (error) { status.value = { success: false, error_message: error.message } } finally { loading.value = false }
}
onMounted(refresh)
</script>
