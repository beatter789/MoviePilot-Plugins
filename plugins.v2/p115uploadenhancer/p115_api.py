...
class P115Api:
    ...
    def __init__(self, client: P115Client, disk_name: str, upload_config: Optional[Dict] = None):
        """
        初始化 115 网盘 API
    ...
    def get_pid_by_path(self, path: Path) -> int:
        ...
        try:
            ...
            logger.warn(f"【115上传增强】获取文件夹ID失败: {str(e)}")
    ...
    def iter_files(self, fileitem: FileItem) -> Optional[List[FileItem]]:
        ...
        try:
            ...
            logger.warn(f"【115上传增强】递归遍历文件夹失败: {str(e)}")
            return None
    ...
    def list(self, fileitem: FileItem) -> List[FileItem]:
        ...
        try:
            ...
            logger.warn(f"【115上传增强】获取信息失败: {str(e)}")
    ...
    def create_folder(self, fileitem: FileItem, name: str) -> Optional[FileItem]:
        ...
        try:
            ...
            check_response(resp)
            logger.info(f"【115上传增强】创建目录: {resp}")
            data = resp.get("cid", resp.get("file_id", None))
            if not data:
                logger.error(f"【115上传增强】创建目录失败: {resp}")
                return None
            ...
            logger.error(f"【115上传增强】创建目录失败: {str(e)}")
            return None
    ...
    def get_folder(self, path: Path) -> Optional[FileItem]:
        ...
        try:
            ...
            logger.error(f"【115上传增强】{path} 目录创建出现未知错误：{e}")
            return None
    ...
    def _get_cached_item(self, path: Path) -> Optional[FileItem]:
        ...
        self._get_item_fail_records.pop(path_str, None)
        logger.debug(f"【115上传增强】缓存获取: {item}")
    ...
    def _query_item(self, path: Path) -> FileItem:
        ...
        logger.debug(f"【115上传增强】文件信息: {file_item}")
        self._get_item_fail_records.pop(path_str, None)
    ...
    def rename(self, fileitem: FileItem, name: str) -> bool:
        ...
        try:
            ...
            logger.error(f"【115上传增强】重命名文件错误: {e}")
            return False
    ...
    def download(self, fileitem: FileItem, path: Path = None) -> Optional[Path]:
        ...
        if not detail:
            logger.error(f"【115上传增强】获取文件详情失败: {fileitem.name}")
            return None
        ...
        if not download_url:
            logger.error(f"【115上传增强】下载链接为空: {fileitem.name}")
            return None
        ...
        # 初始化进度条
        logger.info(f"【115上传增强】开始下载: {fileitem.name} -> {local_path}")
        ...
        try:
            with stream(
                ...
                with open(local_path, "wb") as f:
                    for chunk in r.iter_bytes(chunk_size=10 * 1024 * 1024):
                        if global_vars.is_transfer_stopped(fileitem.path):
                            logger.info(f"【115上传增强】{fileitem.path} 下载已取消！")
                            r.close()
                            return None
                        f.write(chunk)
                ...
                logger.info(f"【115上传增强】下载完成: {fileitem.name}")
        except RequestError as e:
            logger.error(f"【115上传增强】下载网络错误: {fileitem.name} - {str(e)}")
            if local_path.exists():
                local_path.unlink()
            return None
        except Exception as e:
            logger.error(f"【115上传增强】下载失败: {fileitem.name} - {str(e)}")
            if local_path.exists():
                local_path.unlink()
            return None
    ...
    def upload(
        ...
        if not local_path.exists():
            logger.error(f"【115上传增强】本地文件不存在: {local_path}")
            return None
        ...
        if not target_dir_item:
            logger.error(f"【115上传增强】获取网盘目标目录 ID 失败: {target_dir.path}")
            return None
        ...
        # 初始化进度条
        logger.info(f"【115上传增强】开始上传: {local_path} -> {target_path}")
        ...
        try:
            ...
            for init_attempt in range(init_max_retries):
                ...
                try:
                    ...
                    if init_attempt < init_max_retries - 1:
                        ...
                        logger.error(f"【115上传增强】初始化上传重试用尽: {e}")
                        return None
            if init_resp is None:
                return None

            if not init_resp.get("state"):
                logger.error(f"【115上传增强】初始化上传失败: {init_resp.get('error')}")
                return None

            # 检查是否秒传成功
            if init_resp.get("reuse"):
                logger.info(f"【115上传增强】{target_name} 秒传成功")
                progress_callback(100)
            ...
            if not all([bucket_name, object_name, callback_info]):
                logger.error(f"【115上传增强】上传信息不完整: {init_resp}")
                return None
            ...
            logger.info(
                f"【115上传增强】OSS Token 过期时间: {token_expiration.strftime('%Y-%m-%d %H:%M:%S UTC')}"
            )
            ...
            logger.info(
                f"【115上传增强】开始分片上传，分片大小: {part_size // 1024 // 1024}MB"
            )
                ...
                while offset < file_size:
                    ...
                    if self._is_token_expiring(token_expiration, threshold_minutes=5):
                        ...
                        try:
                            ...
                            bucket = Bucket(auth, endpoint, bucket_name)  # noqa
                            logger.info(
                                f"【115上传增强】Token 刷新成功，新的过期时间: "
                                f"{token_expiration.strftime('%Y-%m-%d %H:%M:%S UTC')}"
                            )
                        except Exception as e:
                            logger.error(f"【115上传增强】刷新 Token 失败: {str(e)}")
                            bucket.abort_multipart_upload(object_name, upload_id)
                            return None
                    ...
                    for retry in range(max_retries):
                        try:
                            ...
                            if (
                                ...
                                logger.warn(
                                    f"【115上传增强】检测到 Token 过期错误 ({error_code})，"
                                    f"正在刷新并重试..."
                                )
                                # 刷新 token
                                (
                                ...
                                # 其他错误或重试次数用尽，放弃上传
                                logger.error(f"【115上传增强】上传分片失败: {str(e)}")
                                bucket.abort_multipart_upload(object_name, upload_id)
                                raise
                    ...
                    progress_callback(progress)
                    logger.debug(f"【115上传增强】上传进度: {progress:.1f}%")
            ...
            if result.status == 200:
                logger.info(f"【115上传增强】{target_name} 上传成功")
                return self.get_item(target_path)
            else:
                logger.error(
                    f"【115上传增强】{target_name} 上传失败，状态码: {result.status}"
                )
                return None

        except Exception as e:
            logger.error(f"【115上传增强】上传失败: {local_path} - {str(e)}")
            return None
    ...
    def move(self, fileitem: FileItem, path: Path, new_name: str) -> bool:
        ...
        try:
            ...
            check_response(resp)
            logger.debug(f"【115上传增强】移动文件: {resp}")
            new_path = Path(path) / fileitem.name
